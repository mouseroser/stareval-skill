...
- 有消息能力的 agent：可以向自己的职能群发送开始 / 关键进度 / 完成 / 失败，但仅视为 **best-effort**
- 星鉴中 Step 2 / 3 / 4 / 5 的执行 agent **可以尝试**同时发送到职能群 + 监控群，但这不是可靠前提
- `main`：负责监控群、缺失补发、最终交付、告警，是对外通知的可靠主链路
- 报告类任务完成通知必须携带：标题、结论、推荐路线、主要风险、报告路径
- 群消息完成 ≠ 父链完成；以"文件落地 + 结构化回执 + 通知到位"三者共同成立为准
- **星鉴已验证结论（2026-03-13）**：`sessions_spawn(mode=run)` 下，subagent completion/announce 与 `message(...)` 是独立链路；subagent 可成功完成并回传 announce，但其自发 Telegram 消息可能不投递。因此关键进度不能依赖子 agent 自推。
- **星鉴补充规则**：Step 2 / 3 / 4 / 5 只要已确认"文件落地 + completion/announce 已回主链"，`main` 就应立即补发监控群进度，不等待子 agent 的群消息。
- **监控群职责规则**：子 agent 可以直接发监控群，但监控群可靠可见性必须由 `main` 保证
